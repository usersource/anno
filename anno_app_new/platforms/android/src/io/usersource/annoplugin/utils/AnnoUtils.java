...
public class AnnoUtils {
	 ...
	 * @param gestureViewId
	 */
	public static void setEnableGesture(Activity activity, int gestureViewId, boolean enabled) {
		if (activity != null) {
			GestureOverlayView gestureOverlayView = (GestureOverlayView) activity.findViewById(gestureViewId);
	...
	public static BitmapDrawable rotateImage(BitmapDrawable drawable, int degree) {
		...
		Bitmap rotatedBMP = Bitmap.createBitmap(drawable.getBitmap(), 0, 0, bitmap.getWidth(), bitmap.getHeight(), matrix, true);
	 ...
	 * Display error.
	 * 
	 * This is the universal entry of displaying errors. Just need to change
	...
	/**
	 * trigger Create Anno, take screenshot and launch AnnoDrawActivity it can
	 * be called by JavaScript gesture listener or native calls from host app
	 * 
	 * @param activity
	 *            , source activity
	 */
		...
		try {
			...
			if (!screenshotDir.exists()) {
				if (!screenshotDir.mkdirs()) {
					Log.e(TAG, "Failed to create directory " + screenshotDir.getAbsolutePath());
				}
			}
			...
			screenshotAbsolutePath = screenshotPath.getAbsolutePath();
